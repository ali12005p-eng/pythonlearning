...
async def create_character(message: Message):
    ...
    if uid not in waiting_name:
        ...
        if count % 10 == 0:
            try:
                new_summary = await generate_summary(summary, new_history)
        ...
        asyncio.create_task(
            send_voice_later(response, uid, message)
        )
    ...
    if len(character_name) < 2:
        await message.answer("❌ الاسم قصير جداً")
        return
    ...
    await message.answer("⏳ يتم إنشاء عالمك الخاص...")
