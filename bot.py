...
async def create_character(message: Message):
    ...
    # إذا كان اللاعب موجود مسبقاً => أكمل القصة
...
@dp.callback_query(F.data == "world_map")
async def world_map(callback: CallbackQuery):
    ...
    world_places = places.get(world, ["🌍 أماكن غير مكتشفة"])

    text_msg = f"🗺️ عالمك الحالي\n\n🌍 النوع: {world}\n\n📍 الأماكن المعروفة:\n\n"
    text_msg += "\n".join(world_places)
    text_msg += "\n\n📖 ستُكتشف أماكن جديدة أثناء رحلتك."
